...
function AjouterEvenementForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!token) return setError("Vous devez être connecté pour ajouter un événement.");
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                value={form.lieu}
                onChange={handleChange}
                placeholder="Ex: Salle de conférence, Abidjan"
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2A591D] focus:border-transparent transition-all"
              />
            </div>
...
export default function AjouterEvenementPage() {
  return (
    <Suspense fallback={<div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-[#2A591D]" /></div>}>
